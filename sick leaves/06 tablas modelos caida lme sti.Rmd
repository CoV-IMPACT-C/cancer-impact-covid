...
load(file = "output/tab/tab nb sti/20210223 coef sti datos agregados.RData")

tab.models.agg<-tab.models

load(file = "output/tab/tab nb sti/20210322 coef sti por tipo.RData")

tab.models.type<-tab.models

load(file = "output/tab/tab nb sti/20210315 coef sti por tipo y prevision.RData")

tab.models.type.prev<-tab.models

load(file = "output/tab/tab nb sti/20210322 coef sti por tipo y sexo.RData")

tab.models.type.sexo<-tab.models

load(file = "output/DB/20210223 DB predichos nb sti.RData")

dat.plots.agg<-dat.plots

load(file = "output/DB/20210322 DB predichos nb sti por tipo.RData")

dat.plots.type<-dat.plots

load(file = "output/DB/20210315 DB predichos nb sti por tipo y prevision.RData")

dat.plots.type.prev<-dat.plots

load(file = "output/DB/20210322 DB predichos nb sti por tipo y sexo.RData")

dat.plots.type.sexo<-dat.plots

load(file = "output/DB/20210322 DB predichos nb sti nuevos por tipo.RData")

dat.plots.type.new<-dat.plots

load(file = "output/DB/20210322 DB predichos nb sti continuacion por tipo.RData")

dat.plots.type.cont<-dat.plots

load(file = "output/DB/20210503 DB predichos nb sti nuevos.RData")
...
tab.red.prev <- dat.plots.type.prev %>% 
  ...
  summarise(Cancer_type, dif_FONASA, dif_lb_FONASA, dif_ub_FONASA, dif_ISAPRE, dif_lb_ISAPRE, dif_ub_ISAPRE,
                         var_FONASA, var_lb_FONASA, var_ub_FONASA, var_ISAPRE, var_lb_ISAPRE, var_ub_ISAPRE,
  ...
  summarise(Cancer_type,
            ...
            FONASA_var = paste0(var_FONASA, "% (", var_lb_FONASA, "%-", var_ub_FONASA, "%)"),
            ISAPRE_var = paste0(var_ISAPRE, "% (", var_lb_ISAPRE, "%-", var_ub_ISAPRE, "%)"),
            DIF = paste0(dif, " (", dif_lb, "-", dif_ub, ")")
            )
...
```{r AGREGADAS POR PREVISION y tipo}
tab3.agg.prev <- dat.plots.type.prev %>% 
  filter(pandemia == 1, mod.int== "Insurance") %>% 
  group_by(prevision) %>% 
  summarise(count = sum(count),
            yhat = sum(yhat),
