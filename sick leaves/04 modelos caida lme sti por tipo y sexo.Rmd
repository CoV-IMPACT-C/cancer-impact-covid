---
title: "Modelos de serie de tiempo interrumpida para caída en lme oncológicas por tipo y sexo"
output: 
---
...
dat.tipo.sexo <- lme_modelos_agregados %>% 
...
dat.tipo.sexo <- dat.tipo.sexo %>% filter(!(Cancer_type %in% cancer_sexo))
dat.tipo.sexo <- droplevels(dat.tipo.sexo)

sexo <- levels(dat.tipo.sexo$sexo)
tipo_cancer<- levels(dat.tipo.sexo$Cancer_type)
...
for (i in sexo){
  ...
  for (j in tipo_cancer){
    ...
    sets[[i]][[j]] <- dat.tipo.sexo[sexo == i][Cancer_type == j]
  }
}
...
save(models, file = "/Users/Fran/Dropbox (Personal)/Analisis IMED/lme/output/modelos/nb sti caida lme/20210322 sti caida lme por tipo y sexo.RData")
...
save(tab.models, file = "/Users/Fran/Dropbox (Personal)/Analisis IMED/lme/output/tab/tab nb sti/20210322 coef sti por tipo y sexo.RData")
...
#ggsave(g.coeff, 
#       filename = "/Users/Fran/Dropbox (Personal)/Analisis IMED/lme/output/graf/2020/tipo cancer/coef modelos por tipo y sexo.png", 
#       device = "png",
#       dpi = "retina",
#       width = 12, height = 8)
```
...
save(dat.plots, file = "/Users/Fran/Dropbox (Personal)/Analisis IMED/lme/output/DB/20210322 DB predichos nb sti por tipo y sexo")
```
...
g1.b <- dat.plots %>% 
  group_by(date_wk, sexo, Cancer_type) %>% summarise(count = sum(count),
                                  yhat = sum(yhat),
                                  cf = sum(cf),
                                  ub = sum(ub),
                                  lb = sum(lb),
...
ggsave(g1.b, 
       filename = "/Users/Fran/Dropbox (Personal)/Analisis IMED/lme/output/graf/2020/tipo cancer/serie completa por tipo sexo.png", 
       device = "png",
       dpi = "retina",
       width = 12, height = 8)
```
...
g2.b <- dat.plots %>% 
  ...
  facet_grid(Cancer_type ~ sexo, scales = "free") + 
...
ggsave(g2.b, 
      filename = "/Users/Fran/Dropbox (Personal)/Analisis IMED/lme/output/graf/2020/tipo cancer/serie 2020 vs anterior por tipo y sexo.png", 
       device = "png",
       dpi = "retina",
       width = 12, height = 15)
```
