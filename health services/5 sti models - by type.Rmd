---
title: "Modelos de serie de tiempo interrumpida para caída en prestaciones oncológicas"
output: 
---

#

```{r RMD SETTINGS, message=FALSE, warning=FALSE, include=FALSE}
rm(list = ls()); gc() 
knitr::opts_knit$set(root.dir = "C:/Users/sflor/Documents/Data IMED/Prestaciones/")
knitr::opts_chunk$set(include = FALSE,
                      echo = FALSE,
                      message = FALSE,
                      warning = FALSE)
```

```{r LIBRERIAS, message=FALSE, warning=FALSE, include=FALSE}
library(data.table)
library(tidyverse)
library(MASS)
library(sjPlot)
```

# 1. Datos

```{r LOAD AND PREPARE SAVED DATA GROUPED BY WEEK}
load("C:/Users/sflor/Dropbox/Trabajo/ANID-Covid/Analisis IMED/prestaciones/output/DB/DB para modelos agregada.RData")

prest <- levels(dat.ag[,grupo.trz])
prest.sexo <- as.character(unique(dat.ag[, .(grupo.trz = grupo.trz, grp.cancer = grp.cancer)][
                            !(grp.cancer %in% c("Prostate Cancer", "Breast Cancer", 
                                                "Cervical Cancer"))][
                                       order(grupo.trz)  ][ , grupo.trz]))
dat.ag <- dat.ag[
  , week := as.factor(week)][
#  date_wk < as.Date("2020-08-24")][
  !(grupo.trz == prest[5] & edad_cat == "[0,10)")][
  !(grupo.trz == prest[8] & edad_cat == "[0,10)")][
  !(grupo.trz == prest[15] & edad_cat %in% c("[0,10)", "[10,20)"))][
  , pandemia := cuarentenas][, cuarentenas := NULL][
  , prevision := factor(prevision, labels = c("Public", "Private"))]

# dat.ag[,.(date_wk, year, week, time, time_after, pandemia)][order(time)] %>% unique()
```


# 2. Modelo efectos fijos

```{r MODELO SEXO POR PRESTACION}

sets <- list()

for (i in prest){
    
    sets[[i]] <- dat.ag[grupo.trz == i]
}


formulas <- as.formula(count ~ edad_cat + prevision + year + week + pandemia + time + pandemia:time_after)


models <- list()

for (i in 1:17){
  
  data <- sets[[i]]
    
  models[[i]] <- glm.nb(formulas, data = data)
  
}


# save(models, file = "C:/Users/sflor/Dropbox/Trabajo/ANID-Covid/Analisis IMED/prestaciones/output/modelos/nb sti caida prestaciones/20210310 sti caida prestaciones x prestacion.RData" )

# load(file = "C:/Users/sflor/Dropbox/Trabajo/ANID-Covid/Analisis IMED/prestaciones/output/modelos/nb sti caida prestaciones/20210310 sti caida prestaciones x prestacion.RData")
```

## 2.1 Tablas modelos

```{r}
tab.models <- data.table()

prest.eng <- c("Outpatient Visit", "Antigens\n(Ca 125, Ca 15-3, Ca 19-9, Cea)", "Biopsies", "Neoplastic Cells" , "PET-CT", "FOBT", "Sigmoidoscopy and Colonoscopy", "Cervical Biopsy", "Colposcopy", "Smear (PAP) test", "Upper GI endoscopy ", "Urease test", "Breast US", "Mammography", "Breast MRI", "PSA", "Prostate US")

for (i in 1:17){
  
  summ <- summary(models[[i]])
  mod.res <- as.data.frame(summ$coefficients)
  mod.res$Grupo <- prest.eng[i]
  mod.res$Vars <- rownames(summ$coefficients)

  tab.models <- rbind(tab.models, mod.res)

}

rm(summ, mod.res, i)

confint.models <- data.table()

for (i in 1:17){
  
  conf <- confint(models[[i]])
  conf.res <- as.data.frame(conf)
  conf.res$Grupo <- prest.eng[i]
  conf.res$Vars <- rownames(conf)

  confint.models <- rbind(confint.models, conf.res)
  
}
 
rm(conf, conf.res, i)

tab.models <- cbind(tab.models, confint.models[,c(1:2)])

invlink <- models[[1]]$family$linkinv

. <- tab.models[
  , IRR := invlink(Estimate) ][
  , IC_lower := invlink(`2.5 %`) ][
  , IC_upper := invlink(`97.5 %`) ]

tab.models <- .

# save(tab.models, file = "C:/Users/sflor/Dropbox/Trabajo/ANID-Covid/Analisis IMED/prestaciones/output/tabs_prestaciones/20210310 coef sti prestacion.RData")

# load(file = "C:/Users/sflor/Dropbox/Trabajo/ANID-Covid/Analisis IMED/prestaciones/output/tabs_prestaciones/20210310 coef sti prestacion.RData")
```

## 2.2 Gráfico de coeficientes de modelos

```{r fig.height=10, fig.width=6}
tab.models %>% 
  ggplot(aes(x = IRR, y = Vars)) +
  geom_point()
```

```{r fig.height=10, fig.width=10}
tab.plot <- tab.models %>% 
  filter(Vars %in% c("edad_cat[0,10)", "edad_cat[10,20)", "edad_cat[20,30)", 
                     "edad_cat[30,40)", "edad_cat[50,60)", "edad_cat[60,70)", "edad_cat[70,80)", 
                     "previsionPrivate", "pandemia", "pandemia:time_after", "time")) %>% 
  ggplot(aes(x = IRR, y = Vars)) +
  geom_point(color = "red") +
  geom_errorbarh(aes(xmin = IC_lower, xmax = IC_upper), color = "red", alpha = .5) +
  geom_vline(xintercept = 1, alpha = .3, linetype = 2) +
  facet_wrap("Grupo", ncol = 6, scales = "free_x") +
  theme(legend.position = "bottom")

tab.plot

ggsave(plot = tab.plot,
       filename = "C:/Users/sflor/Dropbox/Trabajo/ANID-Covid/Analisis IMED/prestaciones/output/plots_prestaciones/caida 2020/tab plot by prestacion.png", 
       device = "png",
       dpi = "retina",
       width = 10, height = 10)
```


# 3. Predicciones

```{r PREDICTED DB}
dat.plots.list <- list()

for (i in 1:17){
    
  dat.cf <- sets[[i]]
  dat.cf$pandemia <- 0

  dat.plots.temp <- sets[[i]]

  ilink <- family(models[[i]])$linkinv
  
  pred <- predict(models[[i]], se.fit = T)
  yhat <- pred$fit
  se <- pred$se.fit
  
  dat.plots.temp$yhat <- ilink(yhat)
  dat.plots.temp$ub <- ilink(yhat + (1.96*se))
  dat.plots.temp$lb <- ilink(yhat - (1.96*se))
  
  
  pred.cf <- predict(models[[i]], newdata = dat.cf, se.fit = T)
  cf <- pred.cf$fit
  cf.se <- pred.cf$se.fit
  
  dat.plots.temp$cf <- ilink(cf)
  dat.plots.temp$cf_ub <- ilink(cf + (1.96*cf.se))
  dat.plots.temp$cf_lb <- ilink(cf - (1.96*cf.se))
  
  dat.plots.list[[i]] <- dat.plots.temp
  
  rm(dat.cf, dat.plots.temp, ilink, pred, yhat, se, pred.cf, cf, cf.se)
    
}
```

```{r RECODE AND BIND PREDICTED DB}
for (i in 1:17){
    
  levels(dat.plots.list[[i]]$grp.cancer) <- c("General Oncological Services", 
                                                   "Colorectal Cancer", 
                                                   "Cervical Cancer", "Gastric Cancer", 
                                                   "Breast Cancer", "Prostate Cancer")

  levels(dat.plots.list[[i]]$grupo.trz) <- c("Outpatient Visit", "Antigens\n(Ca 125, 
                                                  Ca 15-3, Ca 19-9, Cea)", 
                                   "Biopsies",   "Neoplastic Cells" , "PET-CT", 
                                 "FOBT", "Sigmoidoscopy and Colonoscopy", 
                                 "Cervical Biopsy", "Colposcopy", "Smear (PAP) test", 
                                 "Upper GI endoscopy ", "Urease test", 
                                 "Breast US", "Mammography", "Breast MRI",
                                 "PSA", "Prostate US")
}

dat.plots <- data.table()

for (i in 1:17){
    
  dat.plots <- rbind(dat.plots, dat.plots.list[[i]])
}

rm(dat.plots.list)

# save(dat.plots, file = "C:/Users/sflor/Dropbox/Trabajo/ANID-Covid/Analisis IMED/prestaciones/output/DB/20210310 DB predichos nb sti x prestacion.RData")

# load("C:/Users/sflor/Dropbox/Trabajo/ANID-Covid/Analisis IMED/prestaciones/output/DB/20210310 DB predichos nb sti x prestacion.RData")
```

# 4. Plots

## 4.1 Serie completa

```{r fig.height=12, fig.width=8}
g1.a <- dat.plots %>% 
  group_by(date_wk, grupo.trz) %>% summarise(count = sum(count),
                                  yhat = sum(yhat),
                                  cf = sum(cf),
                                  ub = sum(ub),
                                  lb = sum(lb),
                                  cf_ub = sum(cf_ub),
                                  cf_lb = sum(cf_lb)) %>% 
  ggplot(aes(x = date_wk, y = count)) +
  geom_point(alpha = .3) +
  geom_line(aes(x = date_wk, y = yhat)) +
  geom_ribbon(aes(ymin = lb, ymax = ub), alpha = .3) +
  geom_line(aes(x = date_wk, y = cf), linetype = 2, size = .3) +
  geom_ribbon(aes(ymin = cf_lb, ymax = cf_ub), alpha = .15) +
  
  facet_wrap("grupo.trz", scales = "free", ncol = 3) +
  
  ylim(0, NA) +
  theme_minimal() + 
  labs(y = "Count", x = "Date", 
       title = "Additive model") +
  geom_vline(xintercept = as.Date("2020-03-15"), linetype = "dotted", color = "red") +
  theme(legend.position = "bottom",
        axis.title = element_text(size = 7),
        axis.text = element_text(size = 6),
        plot.subtitle = element_text(size = 10),
        plot.title = element_text(size = 12))
  
g1.a
```


## 4.2 Exposición

```{r fig.height=12, fig.width=8, warning=F}
g2.a <- dat.plots %>% 
  mutate(week = as.numeric(week)) %>% 
 mutate(exposicion = ifelse(year == 2020, "2020", "2018-2019"),
         cf = ifelse(exposicion == "2020" & pandemia != 0, cf, NA),
         cf_ub = ifelse(exposicion == "2020" & pandemia != 0, cf_ub, NA),
         cf_lb = ifelse(exposicion == "2020" & pandemia != 0, cf_lb, NA) ) %>% 
  group_by(week, year, exposicion, grupo.trz) %>% summarise(count = sum(count),
                                                 yhat = sum(yhat),
                                                 cf = sum(cf),
                                                 ub = sum(ub),
                                                 lb = sum(lb),
                                                 cf_ub = sum(cf_ub),
                                                 cf_lb = sum(cf_lb) ) %>% 
  group_by(week, exposicion, grupo.trz)%>% summarise(count = mean(count),
                                          yhat = mean(yhat),
                                          cf = sum(cf),
                                          ub = mean(ub),
                                          lb = mean(lb),
                                          cf_ub = mean(cf_ub),
                                          cf_lb = mean(cf_lb) ) %>% 
  
  ggplot(aes(x = week, y = count, fill = exposicion))+
  
  geom_point(alpha = .3, aes(color = exposicion), size = 1.5) +
  geom_line(aes(x = week, y = yhat, color = exposicion, linetype = "Observed")) +
  geom_ribbon(aes(ymin = lb, ymax = ub, group = exposicion), alpha = .3) +

  geom_line(aes(x = week, y = cf, color = exposicion, linetype = "Predicted counterfactual"), 
            size = .3, name = "bla") +
  geom_ribbon(aes(ymin = cf_lb, ymax = cf_ub, group = exposicion), alpha = .15) +
  scale_linetype_manual(values = c("Observed" = 1, "Predicted counterfactual" = 2)) +
  
  facet_wrap("grupo.trz", scales = "free", ncol = 3) +
  
  ylim(0, NA) +
  theme_minimal() + 
  labs(y = "Count", x = "Week", fill = "Period", color = "Period", linetype = "Scenario",
       title = "Health Services associated with Cancer") +
  geom_vline(xintercept = 11, linetype = "dotted", color = "red") + 
  theme(legend.position = "bottom",
        axis.title = element_text(size = 7),
        axis.text = element_text(size = 6),
        plot.subtitle = element_text(size = 10),
        plot.title = element_text(size = 12))

g2.a
```


# Guardar gráficos

```{r}
ggsave(plot = g1.a,
       filename = "C:/Users/sflor/Dropbox/Trabajo/ANID-Covid/Analisis IMED/prestaciones/output/plots_prestaciones/caida 2020/serie completa by prestacion.png", 
       device = "png",
       dpi = "retina",
       width = 8, height = 12)

ggsave(plot = g2.a,
       filename = "C:/Users/sflor/Dropbox/Trabajo/ANID-Covid/Analisis IMED/prestaciones/output/plots_prestaciones/caida 2020/serie 2020 vs anterior by prestacion.png", 
       device = "png",
       dpi = "retina",
       width = 8, height = 12)
```
